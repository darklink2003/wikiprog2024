...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $img_usuario = htmlspecialchars($_POST['img_usuario'], ENT_QUOTES, 'UTF-8');
    ...
    $sql = "UPDATE usuario SET usuario = ?, img_usuario = ?, correo = ?, biografia = ?, rango_id = ?, intentos_fallidos = ?, cuenta_bloqueada = ? WHERE usuario_id = ?";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        die('Error en la preparación de la consulta: ' . $conn->error);
    }
    $stmt->bind_param("ssssiisi", $usuario_nombre, $img_usuario, $usuario_correo, $biografia, $usuario_rango_id, $intentos_fallidos, $cuenta_bloqueada, $usuario_id);
